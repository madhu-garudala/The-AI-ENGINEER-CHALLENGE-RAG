...
export default function Home() {
  ...
  const handleFileUpload = async () => {
    if (!pdfFile || !apiKey.trim()) {
      setError('Please select a PDF file and enter your OpenAI API key');
      return;
    }
        ...
        <div className="max-w-2xl mx-auto w-full mb-6">
          <div className="rounded-2xl border border-gray-200 bg-white/90 shadow-sm p-5">
            ...
            <input
              id="api-key"
              type="password"
              className="w-full p-3 text-base text-gray-800 placeholder:text-gray-400 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500/60 bg-white"
              ...
              <div className="space-y-4">
                ...
                <button
                  className={`w-full inline-flex items-center justify-center gap-2 px-5 py-3 text-sm font-medium text-white rounded-lg transition-colors duration-200 shadow-sm ${
                    !pdfFile || !apiKey.trim() || uploading
                      ? 'bg-gray-400 cursor-not-allowed'
                      : 'bg-blue-600 hover:bg-blue-700'
                  }`}
                  onClick={handleFileUpload}
                  disabled={!pdfFile || !apiKey.trim() || uploading}
                >
                  {uploading && (
